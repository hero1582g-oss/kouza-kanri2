...
export const Timeline = ({ accounts, schedules, occurrenceOverrides }: Props) => {
  ...
  const entries = useMemo(() => {
    ...
    // Include every occurrence between the displayed year and today. Without
    // the intervening years, balances in a past/future year would incorrectly
    // restart from the current balance at that year's edge.
    const calculationStart = yearStart < today ? yearStart : today;
    const calculationEnd = yearEnd > today ? yearEnd : today;
    const calculationEntries = expandSchedulesForRange(schedules, calculationStart, calculationEnd, occurrenceOverrides);
    const raw = calculationEntries
      .filter((entry) => entry.date >= yearStart && entry.date <= yearEnd)
      .filter((entry) => accountId === "all" || entry.accountId === accountId);
    const balanceAfter = new Map<string, number>();

    accounts.forEach((account) => {
      const accountEntries = calculationEntries.filter((entry) => entry.accountId === account.id);
