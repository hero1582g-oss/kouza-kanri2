...
export const getDashboardMetrics = (
  ...
  return {
    ...
    next30Expense: next30.filter((entry) => entry.kind === "expense").reduce((sum, entry) => sum + Math.abs(entry.amount), 0),
    next30Income: next30.filter((entry) => entry.kind === "income").reduce((sum, entry) => sum + Math.abs(entry.amount), 0),
    shortageCount: 0,
  };
};
